...
about the local shape of the surface. Principal curvatures taken as a pair
are more informative. The curvedness was introduced to allow to measure 
the amount of curvature in the given point by a single number, instead of a
pair of numbers. The authors defined the curvedness as 
...
in the given point, not about the way the surface is curved in that point.

As an example, we describe the curvedness of sphere and cylinder:
\begin{itemize}
    ...
    the cylinder, curvedness is therefore equal to a half of the inverse of 
    the radius of the cylinder.
\end{itemize}
 
Let us define the curvedness radius as $r_c = \frac{1}{c}$. It can be percieved as
the radius of a sphere, with curvedness $c$.

For the triangulation adaptive to the local curvature of the surface, an input
size of the edge $e$ is not percieved as the approximate side of the triangulation
triangle. Instead, it is percieved as a measure of the level of detail.
...
Let us define the constant of detial as $k_d = \frac{1}{e}$. It can be percieved
...
should be used around that point to achieve the desired detial is 
calculated as the size of the edge which would fit $k_d$--times int $r_c$.
To avoid the edge case in the planar point, a maximum edge size is set.
...
$M_E$ and $P_E$ is half of the height of the equilateral triangle with the edge $E$.
...
We calculate the edge length for the given constant of detial as $\frac{r_c}{k_d}$
...
of the torus is maximal in the inner part of torus and minimal in the outer part of torus.
The resulting mesh for 4 different constants of detial for a torus is displayed on the
Figure \ref{img:58} in the top row. One may note the differece in the sizes of the 
triangles in the inner
part of the torus and in the outer part of the torus.

\begin{figure}[h!]
    \centerline{\includegraphics[scale=0.5]{images/img58}}
    \caption[Adaptive triangulation of a torus]
    {Adaptive triangulation of a torus with 4 different constants of detail (top row).
    Uniform triangulation of a torus with 4 different constants of detail (bottom row).}
    %id obrazku, pomocou ktoreho sa budeme na obrazok odvolavat
    \label{img:58}
\end{figure}

For the contrast, the unfiform triangulation of the same torus for 4 different edge lengths
is displayed on the Figure \ref{img:58} in the bottom row. One may note that the 
triangles in the inner part of the torus are approximately of the same 
size as the triangles in the outer part of the torus.
...
created around the singularity, we perform binary search to find
...
reflection to create the remainding points on the surface.

In case of other singularities, we create different number of triangles:
\begin{itemize}
...
To find the point of surface, we perform an angular binary search between $\vec{t}$
...
TODO rewrite this odsek
For the singularities with symmetrical branches, we only need to perform binary
search for one of the branches. In case of the singularities with rotation
symmetry, we again create the remainding points by using the rotation symmetry.
In case of other singularities, we again make use of the
potential planes of symmetry.

\subsubsection*{Layers for ADE singularities}
We already introduced the concept of creating multiple layers of triangles around
$A_{n--}$ singularities. In this section we explain the extension of this concept
to remainding ADE singularities. For a given singularity, we create $l$ layers of
points for each branch. In contrast with $A_{n--}$ singularities, we do not 
...
in insufficient approximation of the curves lying in the intersection of the surface 
and coordinate planes. In some cases, it even results in incorrect local mesh with 
overlapping triangles.

On the Figure \ref{img:56} on the left, one may see the insufficient approximation
...
approach is displayed on the Figure \ref{img:D4-before}. One may see that the
outer edges of the triangles have a significant length differece.
