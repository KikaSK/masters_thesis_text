...
$F(x,y,z)=z^{n+1}\pm x^2\pm y^2$, $A_{n-+}$
...
\subsection*{Local triangulation of ADE singularities}
For given edge size $e$, we want to create local triangulation of ADE
singularities, such that edges on the border of the local triangulation
have aproximate length $e$.
\subsubsection*{$A_n$ singualrities}
...
\begin{figure}
    ...
    \label{img:11}
\end{figure}
Given by equation $z^{n+1}-x^2-y^2=0$, we will find the distance of the 
plane $P$ from the plane $x=0$ for the given length $e$ of the sides of
the hexagon.

Let's $e$ be the length of the side of the hexagon.
